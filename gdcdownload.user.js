...
// @match        https://*.gdcvault.com/play/*
// @connect      gdcvault.com
// @connect      blazestreaming.com
// @grant        GM_xmlhttpRequest
// @license MIT
// ==/UserScript==


// ref: https://www.gamedev.net/forums/topic/717577-gdc-vault/
// A simple fetch-like wrapper for Tampermonkey's cross-origin request tool
    ...
    if (window.top === window.self) {
        ...
        */
        function M3U8() {
            ...
            this.start = function(m3u8, options) {
                ...
                var download = new Promise(function(resolve, reject) {
                    ...
                    //m3u8 = m3u8.replace('cdn-a', 'gdcvault')
                    var req = gmFetch(m3u8, 'text')
                    .then(function(d) {
            ...
            function RecurseDownload(arr, cb, i, data) { // recursively download asynchronously 2 at the time
                ...
                function recurseDownload(arr, cb, i, data) {
                    var req = Promise.all([
                        gmFetch(arr[i], 'blob'),
                        arr[i + 1] ? gmFetch(arr[i + 1], 'blob') : Promise.resolve()
                    ])
                    .then(function(d) {
                        // Filter out any undefined elements if the second promise resolved empty
                        return filter(d, function(v) { return v !== undefined; });
                    })
                    .then(function(d) {
        ...
        window.addEventListener("message", async function(event) {
            if (event.data.videoURL == null) return;
            try {
                const text = await gmFetch(event.data.videoURL, 'text');

                // Split manifest text into individual lines and clean them up
                const lines = text.split("\n").map(line => line.trim());

                // Filter out comments/tags (#) and empty lines to find the raw URLs/paths
                const manifestUrls = lines.filter(line => line.length > 0 && !line.startsWith('#'));

                // Take the last valid manifest path available
                const streamPath = manifestUrls.length > 0 ? manifestUrls[manifestUrls.length - 1] : null;

                if (!streamPath) {
                    console.error("Could not extract a valid stream path from the manifest.");
                    return;
                }

                // Replace index.m3u8 with the proper variant sub-playlist location
                url = event.data.videoURL.replace(/index[^\/]*\.m3u8/, streamPath);
                console.log("Constructed Stream URL: " + url);

            } catch (e) {
                console.error("Failed to read cross-origin manifest metadata: ", e);
            }
        }, false);
